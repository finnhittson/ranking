...
def listwise_ranking(X, y, alpha, tolerance):
	...
	while likelihood_loss > tolerance and iterations < 1:
		...
		if iterations % 10 == 9 or True:
			print("Likelihood Loss: {}".format(likelihood_loss))
		iterations += 1
	return clf #W1, b1, W2, b2


def compute_likelihood_loss(clf, W1, b1, W2, b2, X, y):
	p_y_xg = 1
	for i, x in enumerate(X):
		#_, A2 = ann.forward_propagation(clf, W1, b1, W2, b2, np.array([x]).T)
...
if __name__ == '__main__':
	...
	x = [[i] for i in range(-100, -75)] + [[i] for i in range(75, 100)]
